...
//自定义库
use PacketHunter::packet_owned::{Codec, PacketOwned};
use PacketHunter::tools::*;
use PacketHunter::count::{PacketCount, GLOBAL_COUNT_MESSAGE};
...
//并在线程中不断的捕获数据包发往前端
#[tauri::command]
fn start_capture(bpf:String,devname:String,limit:u32,window:Window,active_bank:State<Bank>,cap_run:State<RunState>,cap_handle:State<ThreadState>)->Result<(),String>{
    ...
    .open()
    {
      ...
      let packets;
      {
        ...
        *cap_run.is_running.lock().unwrap() = true;
        run_state = cap_run.is_running.clone();
        packets = active_bank.packets.clone();
        active_bank.linktype.lock().unwrap().replace(cap.get_datalink());
      }
      let global_count;
      {
        //统计块
        global_count = GLOBAL_COUNT_MESSAGE.clone();
      }
      let mut codec = Codec{};
      let newhandle = thread::spawn(move ||{
        let mut last_instant = Instant::now();
        let mut index = 0;
        let mut flow_count  = 0;//流量统计 
        let linktype = cap.get_datalink();
        loop {
          if !(*run_state.lock().unwrap()) || index >= limit{
            let _ = window.emit("Stop", ());
            break;
          }
          let now = Instant::now();
          if now.duration_since(last_instant) >= Duration::from_secs(1){
              global_count.lock().unwrap().push_flow(flow_count);
              last_instant = now;
              flow_count = 0;
          }
          if let Ok(packet) = cap.next_packet(){
            let new_packet = codec.decode(packet);
            flow_count += new_packet.header.caplen;
...
#[tauri::command]
fn get_count()->PacketCount{
  // println!("Get Count!");
  GLOBAL_COUNT_MESSAGE.lock().unwrap().clone()
}
#[tauri::command]
fn clear_count(){
